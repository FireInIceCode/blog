...
$f_{i, j}$ 表示后 $i$ 个数操作了 $j$ 次, 则当前位置可以操作 $t_i=(a_i+j)/i$ 次, $f_{i, j+(a_i+j)/i}=f_{i+1, j}$
...
首先理解一下题意, 可以把 $t-h\le k$ 相当于先给一条链每个点选一个且是免费的, 然后再选剩下的是付费的, 答案可以认为是不包含这条链的背包和这条链上让 $a_i: = a_i-1$ 的背包合并.
