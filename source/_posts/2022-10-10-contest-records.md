...
> ##### 算法三
> 
> 对于 $k=2$ 的情况, 我们假设基本方案为一直待在 $0$ 号城市, 考虑最大化实际方案减去基本方案的效用差. 定义 $d_i=a_{1, i}-a_{0, i}$. 可以发现, 问题相当于找到 $\lfloor \frac{m}{2}\rfloor$ 个区间, 最大化这些区间内 $d$ 的和. 总的来说, 需要解决的问题是, 选中若干个互不相交且不相连的区间使得这些区间元素和之和最大.
> 
> 这是一个经典问题, 可以证明, $i$ 段的最优方案可以从 $i-1$ 段的最优方案反转一个区间内的元素被选/不被选状态得到. (或者, 可以看作模拟费用流的结果. )因此, 从小到大枚举段数(乘火车次数), 用线段树等方式维护区间和, 每次选中全局最大区间和并将区间内元素的效用乘 $-1$(翻转选择状态)即可.
> 
> 时间复杂度: $O(n\log n)$
> 
> 预计得分: $25$, 加上之前共 $50$.
...
感觉算法3是容易想到的, 然后容易猜到凸性配合wqs二分得到算法4, 但算法5还是很厉害.

## ZSTEST230831

### A

基本就是CF Make It One, 不过范围是 $4\times 10^6$, 用gcd卷积那一套即可. 复杂度应该能到 $v\log \log v \log \log \log v$. 瓶颈是二分答案里面快速幂.
...
直接找到区间中第一个最大值$A_x$,则全局中$i\in [l,x]$开始往后找到的最大子序列$f_i$和限制了区间之后找到的只差一个$f_i-1$,因为走到$i$之后的剩下都相同了.而$i\in [x+1,r]$可以找以它结尾的值$g_i$,那么它一定起始于$x$之后也就一定在区间里.于是最后只需要预处理$f,g$和最大值的ST表,查询的时候就只有RMQ了.