var dict = {
    '_': 'operation:left click,right click,middle click,drag with mouse.<br>commands: `set-size` `show-as-tree` `fix-all` `attr`',
    'set-size': '`set-size <width:int> <height:int>`: set the size of canvas ',
...
function parseCommand(str, graph) {
    ...
    if (words[0] == 'set-size') {
        ...
        if (words.length == 2) {
            G.Config[words[1]] = eval(G.Config[words[2]]);
        } else {
